...
the member name. */
SELECT DISTINCT
    CONCAT(surname, ' ', firstname) AS member_name, f.name AS facility
FROM
    Members AS m
INNER JOIN
    Bookings AS b
...
SELECT m.months, COUNT(m.memid) AS m_usage
FROM (
